...
public class PoolSimulator_SLiM_bacteria {
	...
	public PoolSimulator_SLiM_bacteria(String parameter_file) throws IOException {
...
//            	}
                ...
                for(int h=0;h < actual_num_haps;h++) {
                	...
                	if(pool2allhapList.get(p).containsKey(curr_hap)) {
                		for(int currhap_num=0;currhap_num<pool2allhapList.get(p).get(curr_hap);currhap_num++) {
                    		...
                    		for(int v = 0; v < actual_num_vars; v++){
		        				String Chrom = "Haplotype_" + h+"_"+currhap_num;
		        				int pos_start = sim_var_pos[v]-750;
		        				if (pos_start<1) {
		        					pos_start=1;
		        				}
		        				int pos_end= sim_var_pos[v]+750;
		        				if (pos_end>this.ref_seq_len) {
		        					pos_end= this.ref_seq_len;
		        				}
		        				
		        				bw_bed.write(Chrom+"\t" +  pos_start+ "\t"+ 
		        						pos_end+"\n");
		        			}
                		}
                	}
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		PoolSimulator_SLiM_bacteria ps=new PoolSimulator_SLiM_bacteria(parameter);
